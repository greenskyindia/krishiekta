...
#				except:
	...
	def search_fn0(self,art_uid, art_url, src_tag_list):
		...
		try:	
			while (num <= ListLength+1):
				print("---Inside while with num = " + str(num))
				numdash = num
				if(src_tag_list[num] == temp.tcombined_name):
					print("--Inside if with values: tag list value- " + src_tag_list[num])
					print("and element tag" + temp.tcombined_name)
					for j in temp.tchild:
						print("--TID of child value: " + str(j))
						print("--Name of child value: " + Tree[j].tcombined_name)
						print("--Searching for the tag from list: " + src_tag_list[num+1])
						if(Tree[j].tcombined_name == src_tag_list[num+1]):
							f = Tree[j].tid
							print("--TID of new element: " + str(f))

							temp = Tree[f]

							print("--TID of new element: " + str(temp.tid))
							print("--Combined name of new elemebt: " + temp.tcombined_name)
				...
				#Error handling when no element found in a loop. Prevents infinite loop.
				if(numdash == num):
	...
	def search_fn1(self,art_uid, art_url, src_tag_list, attr):
		...
		print (temp.tchild)
		for p in temp.tchild:
			print(Tree[p].tcombined_name)
	
		print(src_tag_list)
		print(temp.tcombined_name)

		if(src_tag_list[0] == temp.tcombined_name):
