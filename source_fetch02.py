	...
	def article_collection(self):
		for i in self.source_list:
			if(i.gen_page_type == 'XML'):
				...
				send_date = datetime_now.strftime("%Y-%m-%d %H:%M:%S")
				send_date = '"' + send_date + '"'
				last_date = '"' + str(i.last_fetch_date) + '"'
			
				update_dict = [('last_fetch_date',send_date),('last_base_date',last_date)]
				where_string = "id = " + str(i.source_id)

				self.db_handler.update_table("source", update_dict, where_string)
		...
		if(self.source_list[src_indx].gen_tag_list['url']!=None):
			...
			for i in art_list:
				...
				try:
					...
					daylight_date_format = "%A %b %d %H:%M:%S EDT %Y"
					temp_date = datetime.strptime(i['date'], daylight_date_format)
...
#				tdate = datetime.strptime(self.source_list[src_indx].last_fetch_date, "%Y-%m-%d %H:%M:%S")
#				odate2 = odate.strftime("%a, %d %b %Y %H:%M:%S ")

#				tdate = self.source_list[src_indx].last_fetch_date.strftime("%a, %d %b %Y %H:%M:%S ")
				tdate = self.source_list[src_indx].last_fetch_date
				...
				if temp_date>tdate:
					...
					self.db_handler.insert_into("raw_data",["uid","source_id","url","date","title","content","cge_ready"],temp_list)
